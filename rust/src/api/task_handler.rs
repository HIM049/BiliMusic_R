...
use crate::{app_state::{self, APP_STATE}, task_modules::Task};
...
pub fn task_queue_filter(queue: Vec<Task>, options: FilterOptions) -> Vec<Task> {
    if options.is_with_parts {
        ...
        queue.iter()
            .enumerate()
            // .filter(|(index, _)| *index >= options.from.try_into().unwrap() && *index <= options.to.try_into().unwrap())
            .filter(|(_, task)| task.part_id == 0)
